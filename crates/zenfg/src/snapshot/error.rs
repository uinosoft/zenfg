/// Failure to adapt a native compilation report to Snapshot 1.2.
#[non_exhaustive]
...
pub enum SnapshotExportError {
    ...
    /// Native texture usage contains no Snapshot 1.2 representation.
    ...
    /// Native buffer usage contains no Snapshot 1.2 representation.
    ...
    /// A native enum or descriptor value cannot be expressed by Snapshot 1.2.
    ...
    /// Optional execution timing data belongs to a different caller frame.
    #[error("Timing frame {timing_frame} does not match capture frame {capture_frame}")]
    TimingFrameMismatch {
