...
export function createPresenter(device: GPUDevice, format: GPUTextureFormat) {
    const module = device.createShaderModule({ label: 'three-interop.present.shader', code: presentShader });
    const pipeline = device.createRenderPipeline({
        label: 'three-interop.present.pipeline',
        layout: 'auto',
