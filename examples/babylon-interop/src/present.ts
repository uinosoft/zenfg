...
export function createPresenter(device: GPUDevice, format: GPUTextureFormat) {
    const module = device.createShaderModule({ label: 'babylon-interop.present.shader', code: presentShader });
    const pipeline = device.createRenderPipeline({
        label: 'babylon-interop.present.pipeline',
        layout: 'auto',
