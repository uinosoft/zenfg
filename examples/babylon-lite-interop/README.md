...
Run `npm run dev:playground` and select **Babylon Lite Co-rendering**, or use
...
Import `startBabylonLiteInterop` from `@zenfg-example/babylon-lite-interop`, passing
...
```sh
npm test --workspace @zenfg-example/babylon-lite-interop
npm run typecheck
npm run build:pages
npm run docs:check
```
