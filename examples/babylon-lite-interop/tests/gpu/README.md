...
```sh
node examples/babylon-lite-interop/tests/gpu/run.mjs
```
...
After `npm run build:pages`, serve `.pages` with Vite preview and run:

```sh
node examples/babylon-lite-interop/tests/gpu/playground.mjs
```

`PLAYGROUND_URL` defaults to `http://127.0.0.1:4175/playground/`. This checks actual
