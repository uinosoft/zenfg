...
export interface BabylonLiteInteropController {
...
export function createHostSupport(canvas: HTMLCanvasElement,
    device: GPUDevice,
    graph: FrameGraph,
    bridge: BabylonLiteBridge,
    options: StartBabylonLiteInteropOptions,
    ...
    // Lite collects input synchronously; the queued frame consumes it once before graph recording.
    const inputEvents = ['pointerdown', 'pointermove', 'pointerup', 'pointercancel', 'wheel', 'keydown', 'touchstart', 'touchmove', 'touchend'] as const;
    function restoreOutline(): void { canvas.style.outline = previousOutline; }
    function input(event: Event): void {
        // Keep pointer interaction unobtrusive while preserving keyboard focus.
    ...
    const controller: BabylonLiteInteropController = {
        captureSnapshot() {
    ...
    void device.lost.then(info => fail(new Error(`Babylon Lite co-rendering WebGPU device was lost: ${info.message || info.reason}`)));
...
export function notifyError(options: StartBabylonLiteInteropOptions, error: unknown): void {
