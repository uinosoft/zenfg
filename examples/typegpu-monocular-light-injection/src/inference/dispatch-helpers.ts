...
import type { TgpuBuffer, TgpuComputePipeline, TgpuRoot, TgpuUniform, ValidateUniformSchema } from 'typegpu';
...
export interface DispatchContext {
  ...
  readonly uniformBuffers: GPUBuffer[];
  readonly mutableStorageBuffers: GPUBuffer[];
  readonly readonlyStorageBuffers: GPUBuffer[];
  readonly own: <T extends OwnedGpuResource>(resource: T) => T;
  readonly ownStorage: <T extends TgpuBuffer<any>>(resource: T) => T;
...
export function createDispatchContext(
  ...
  const mutableStorageBuffers: GPUBuffer[] = [];
  const readonlyStorageBuffers: GPUBuffer[] = [];
  ...
  return {
    ...
    ownedResources,
    uniformBuffers,
    mutableStorageBuffers,
    readonlyStorageBuffers,
    own,
    ownStorage: <T extends TgpuBuffer<any>>(resource: T) => {
      own(resource);
      mutableStorageBuffers.push(root.unwrap(resource));
      return resource;
    },
    uniform: <TSchema extends d.AnyWgslData>(
      ...
      own(buffer.buffer);
      uniformBuffers.push(root.unwrap(buffer.buffer));
      return buffer;
    },
    storageFromBytes: (bytes) => {
      ...
      const buffer = root.unwrap(resource);
      readonlyStorageBuffers.push(buffer);
      return buffer;
    },
