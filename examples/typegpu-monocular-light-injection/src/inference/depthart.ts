...
export class DepthInferencePlan {
  ...
  readonly #uniformBuffers: readonly GPUBuffer[];
  readonly #mutableStorageBuffers: readonly GPUBuffer[];
  readonly #readonlyStorageBuffers: readonly GPUBuffer[];
  ...
  constructor(root: TgpuRoot, bundle: DepthBundle) {
    ...
    this.#uniformBuffers = prepared.uniformBuffers;
    this.#mutableStorageBuffers = prepared.mutableStorageBuffers;
    this.#readonlyStorageBuffers = prepared.readonlyStorageBuffers;
  ...
  get dispatchCount(): number {
    return this.#dispatches.length + 1;
  }

  get arenaBuffers(): readonly GPUBuffer[] {
    return this.#arena.allocationBuffers;
  }

  get weightBuffers(): readonly GPUBuffer[] {
    return [...this.#weights.buffers.values()].map((buffer) => buffer.buffer);
  }

  get uniformBuffers(): readonly GPUBuffer[] {
    return [this.#preprocessor.uniformBuffer, ...this.#uniformBuffers];
  }

  get mutableStorageBuffers(): readonly GPUBuffer[] {
    return this.#mutableStorageBuffers;
  }

  get readonlyStorageBuffers(): readonly GPUBuffer[] {
    return this.#readonlyStorageBuffers;
  }
  ...
  encodeFrame(pass: GPUComputePassEncoder, frame: GPUExternalTexture, options: DepthFrameOptions): void {
