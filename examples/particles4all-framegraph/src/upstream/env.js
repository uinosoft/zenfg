...
function decodeHDR(bytes) {
  ...
  let p = 0;
  const line = () => {
    if (p >= u8.length) throw new Error('.hdr ended early');
    let s = '';
  ...
  if (!Number.isInteger(w) || !Number.isInteger(h) || w <= 0 || h <= 0 || w * h > 67108864)
    throw new Error('invalid or oversized .hdr dimensions');
    ...
    if (r === 2 && g === 2 && ((b << 8) | e) === w && w >= 8 && w < 32768) {
      ...
      for (let c = 0; c < 4; c++) {
        ...
        while (x < w) {
          ...
          if (!count || x + count > w || p + (n > 128 ? 1 : n) > u8.length)
            throw new Error('invalid .hdr scanline');
          if (n > 128) {
            n -= 128;
            const v = u8[p++];
      ...
      while (x < w) {
        ...
        if (rr === 1 && gg === 1 && bb === 1) {
          const n = ee << (shift * 8);
          if (!x || n <= 0 || shift > 3 || x + n > w) throw new Error('invalid .hdr run');
          const prev = (x - 1) * 4;
          for (let k = 0; k < n; k++, x++)
...
async function decodeImage(blob) {
  ...
  try {
    ...
    if (!ctx) throw new Error('Unable to decode the panorama image.');
...
export class Environment {
  ...
  async load(src) {
    if (this.destroyed) throw new Error('Particles4All environment is disposed.');
    ...
    try {
      ...
      if (src instanceof Blob) {
        ...
        img = /\.hdr$/i.test(name) || new Uint8Array(buf)[0] === 35 ? decodeHDR(buf) : await decodeImage(src);
      } else {
        name = String(src);
        const res = await fetch(name, { signal: controller.signal });
        if (!res.ok) throw new Error(`${name}: ${res.status} ${res.statusText}`);
        if (/\.hdr(?:[?#].*)?$/i.test(name)) img = decodeHDR(await res.arrayBuffer());
        else img = await decodeImage(await res.blob());
      }
      if (this.destroyed || request !== this.request) return this.status;
      const { w, h, data } = img;
      if (w < 2 * h - 2 || w > 2 * h + 2)
        throw new Error(`${name} is ${w}x${h} - an equirectangular panorama is 2:1`);

      const size = Math.min(2048, this.dev.limits?.maxTextureDimension2D ?? 2048, Math.max(16, Math.floor(w / 4)));
      const levels = Math.floor(Math.log2(size)) + 1;
      texture = this.dev.createTexture({
      ...
      for (let f = 0; f < 6; f++) {
        ...
        for (let m = 0; m < levels; m++) {
          this.dev.queue.writeTexture(
            { texture, mipLevel: m, origin: [0, 0, f] },
  ...
  clear() {
    ...
    this.abortController?.abort();
    this.abortController = null;
    const empty = this.dev.createTexture({
      size: [1, 1, 6], format: 'rgba16float', label: 'envCubeEmpty',
      usage: GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_DST });
    const view = empty.createView({ dimension: 'cube' });
