...
const MODEL_VARIANTS: Record<ModelSize, { readonly fp16: ModelVariant; readonly f32?: ModelVariant }> = {
    small: {
...
const MODEL_HOST = `https://huggingface.co/reczkok/depthart-typegpu/resolve/${MODEL_REVISION}`;
const MODEL_CACHE = `zenfg-depthart-models-${MODEL_REVISION}`;
const CACHE_OPT_OUT_KEY = 'zenfg.depthart-cache-disabled';
...
export function setCachingEnabled(enabled: boolean): void {
    try {
        if (enabled) localStorage.removeItem(CACHE_OPT_OUT_KEY);
        else localStorage.setItem(CACHE_OPT_OUT_KEY, '1');
    } catch {
        // Cache Storage remains best-effort in private browser contexts.
    }
}
...
export async function clearDownloads(): Promise<void> {
    try {
        ...
        // Nothing to clear when Cache Storage is unavailable.
    }
}
