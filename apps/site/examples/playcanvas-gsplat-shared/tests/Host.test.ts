...
import { startPlayCanvasGsplatInterop } from '../../playcanvas-gsplat-interop/src/main.ts';
...
function stubBridge(device: GPUDevice, width: number, height: number) {
    ...
    const allocate = (w: number, h: number) => ({
        color: device.createTexture({ format: 'rgba8unorm', size: [w, h], usage: GPUTextureUsage.RENDER_ATTACHMENT | GPUTextureUsage.TEXTURE_BINDING }),
    ...
    const stub = {
        device,
        syncCamera() {}, renderedSplats: 9, loadingCount: 0, setBudget() {},
    ...
    return { bridge: stub as unknown as PlayCanvasSplatBridge, get destroyed() { return destroyed; } };
}
...
test('host renders continuously, captures real frames, and destroys borrowers before the device owner', async t => {
    ...
    const create = t.mock.method(PlayCanvasSplatBridge, 'create', async () => stub.bridge);
    try {
        const errors: Error[] = [];
        const controller = await startPlayCanvasGsplatInterop(host.canvas, { onError: error => errors.push(error) });
        ...
        assert.ok(snapshot);
        assert.deepEqual(snapshot.graph.nodes.map(node => node.label), ['Reset', 'Cull', 'Draw', 'playcanvas-gsplat-interop.playcanvas', 'playcanvas-gsplat-interop.composite-present']);
        ...
        assert.equal(host.trace.submits, submissions + 9, 'frames submit without input or capture requests');

        assert.equal(create.mock.callCount(), 1);
        host.resize(500, 200); host.flushFrame();
        assert.ok(host.trace.destroyedTextures.some(texture => texture.label === 'playcanvas-gsplat-interop.reference-color'), 'resize destroys the previous pooled color attachment');
...
test('late startup after cancellation releases the bridge without reporting an error', async t => {
    const host = installHost();
    const stub = stubBridge(host.device, 320, 180);
    let finish = (_: PlayCanvasSplatBridge) => {};
    t.mock.method(PlayCanvasSplatBridge, 'create', () => new Promise<PlayCanvasSplatBridge>(resolve => { finish = resolve; }));
    try {
        const abort = new AbortController();
        const errors: Error[] = [];
        const starting = startPlayCanvasGsplatInterop(host.canvas, { signal: abort.signal, onError: error => errors.push(error) });
...
for (const end of ['abort', 'device loss', 'submission error'] as const) {
    test('hidden host settles captures and cleans up on ' + end, async t => {
        const host = installHost();
        const stub = stubBridge(host.device, 320, 180);
        t.mock.method(PlayCanvasSplatBridge, 'create', async () => stub.bridge);
        try {
            const abort = new AbortController(); const errors: Error[] = [];
            const controller = await startPlayCanvasGsplatInterop(host.canvas, { signal: abort.signal, onError: error => errors.push(error) });
            assert.ok(controller);
...
test('startup failure is reported once and never schedules a frame', async t => {
    const host = installHost();
    t.mock.method(PlayCanvasSplatBridge, 'create', async () => { throw new Error('preparation failed'); });
    try {
        const errors: Error[] = [];
        assert.equal(await startPlayCanvasGsplatInterop(host.canvas, { onError: error => errors.push(error) }), undefined);
