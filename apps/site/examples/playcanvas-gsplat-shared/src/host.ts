...
import { ExampleCamera } from './camera.ts';

export interface StartSplatOptions {
...
export function createHostSupport(canvas: HTMLCanvasElement,
    device: GPUDevice,
    graph: FrameGraph,
    bridge: ExampleCamera,
    options: StartSplatOptions,
    ...
    // Input is consumed once before recording the next graph frame.
    ...
    function suspend(): void {
        state.suspended = true; bridge.clearInput();
    ...
    void device.lost.then(info => fail(new Error(`PlayCanvas GSplat co-rendering WebGPU device was lost: ${info.message || info.reason}`)));
...
export function notifyError(options: StartSplatOptions, error: unknown): void {
