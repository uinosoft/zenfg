...
export function createPresenter(device: GPUDevice, format: GPUTextureFormat) {
    const module = device.createShaderModule({ label: 'babylon-lite-interop.present.shader', code: presentShader });
    const pipeline = device.createRenderPipeline({
        label: 'babylon-lite-interop.present.pipeline',
        layout: 'auto',
    ...
    return (frame: FrameGraphRecording, scene: TextureHandle, backbuffer: TextureHandle): void => {
        ...
        frame.render({
            label: 'babylon-lite-interop.present',
            uses: [sample],
