...
export class Renderer {
  ...
  prepare(sim, cam, opts, particleCount = sim.n) {
    const eye = cam.eye();
    ...
    if (ssfrOn) mesh.configure([0, 0, 0], sim.params.box, opts.meshRes, 200000);
    const invViewProj = inverse(vp);
    const invView = [right[0], right[1], right[2], 0,
                     up[0], up[1], up[2], 0,
                     -fwd[0], -fwd[1], -fwd[2], 0,
                     eye[0], eye[1], eye[2], 1];
    return { eye, proj, viewM, vp, invViewProj, invView, mesh, solids,
             ssfrOn, meshOn, rayOn, right, up, fwd, particleCount };
  }

  encodeSurface(enc, sim, opts, frame) {
    if (!frame.meshOn) return;
    if (frame.rayOn && opts.raySurface === 0) {
      frame.mesh.generateField(enc, sim, opts.meshIso);
      frame.mesh.smoothField(enc, sim, opts.meshIso, opts.fieldSmooth);
    } else {
      frame.mesh.generate(enc, sim, opts.meshIso, opts.fieldSmooth);
    }
  }

  encodeSolidPacking(pass, sim, frame, packed) {
    if (frame.solids) frame.solids.pack(pass, sim, packed);
  }

  encodeRaster(pass, sim, frame, packed, surface) {
    if (frame.meshOn) {
      frame.mesh.draw(pass, this.uni, surface.vertices, surface.indirect);
      if (frame.solids) frame.solids.draw(pass, this.uni, packed);
    } else {
      pass.setPipeline(this.pipe);
      pass.setBindGroup(0, this.simBind[frame.parity]);
