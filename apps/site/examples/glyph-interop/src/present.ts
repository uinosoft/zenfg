...
export function createPresenter(device: GPUDevice, format: GPUTextureFormat) {
    const module = device.createShaderModule({ label: 'glyph-interop.present.shader', code: presentShader });
    const pipeline = device.createRenderPipeline({
        label: 'glyph-interop.present.pipeline',
        layout: 'auto',
