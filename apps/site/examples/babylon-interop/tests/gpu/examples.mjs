// Run against the built Examples preview, e.g. EXAMPLES_URL=http://127.0.0.1:4173/playground/.
...
const url = new URL(process.env.EXAMPLES_URL ?? 'http://127.0.0.1:4173/playground/');
...
try {
    ...
    await ready();
    await page.screenshot({ path: resolve(output, 'examples-desktop.png') });
    ...
    await page.screenshot({ path: resolve(output, 'examples-pointer-focus.png') });
    await page.mouse.up();
    ...
    const snapshotPath = resolve(output, 'examples-inspector.snapshot.json');
    ...
    await page.screenshot({ path: resolve(output, 'examples-inspector.png') });
    ...
    await writeFile(resolve(output, 'examples-result.json'), JSON.stringify(result, null, 2));
    console.log(JSON.stringify(result));
} catch (error) {
    await writeFile(resolve(output, 'examples-result.json'), JSON.stringify({ ok: false, error: String(error), errors, remoteRequests }, null, 2));
    throw error;
