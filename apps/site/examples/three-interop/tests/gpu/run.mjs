...
const output = resolve(root, '.test-dist/three-interop-gpu');
await mkdir(output, { recursive: true });
const bundle = await build({
    absWorkingDir: root,
    entryPoints: ['apps/site/examples/three-interop/tests/gpu/browser.ts'],
...
const server = createServer((request, response) => {
    if (request.url === '/suite.js') {
        ...
        response.end('<!doctype html><title>Three interop hardware WebGPU suite</title><body><pre id="result">Running hardware WebGPU tests…</pre><script type="module" src="/suite.js"></script>');
...
try {
    ...
    await page.waitForFunction(() => globalThis.__threeInteropGpuResult !== undefined, undefined, { timeout: 180_000 });
    const { images = {}, snapshots = {}, ...result } = await page.evaluate(() => globalThis.__threeInteropGpuResult);
