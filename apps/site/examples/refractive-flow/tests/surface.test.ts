...
test('captures the optical graph and preserves one renderer across theme, input and visibility changes', async () => {
	...
	try {
		...
		assert.equal(uniformValues()[2], 0, 'viewpoint starts centered');
		...
		aim('touch'); frame(325); assert.equal(uniformValues()[2], 0, 'touch does not move the viewpoint');
		aim('mouse'); frame(330);
		const parallax = uniformValues()[2]!;
		assert.ok(parallax > 0 && parallax < 24 / 64, 'mouse position eases toward a bounded offset');
		assert.ok(uniformValues()[3]! < 0, 'screen-down motion is converted to clip coordinates');
		...
		assert.equal(uniformValues()[2], 0); assert.equal(Math.abs(uniformValues()[3]!), 0);
