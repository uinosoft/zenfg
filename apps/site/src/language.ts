...
const translations: Record<Language, Record<string, string>> = {
	en: {
		...
		examplesBoundary: "These are version-specific integration examples, not a promise of compatibility with every engine version.",
		...
		runtimeBoundary: "The runtimes share semantics and portable snapshots, with idiomatic APIs for each language. They do not share live GPU resources across runtimes.",
		visionTitle: "Toward reusable rendering building blocks",
		visionBody: "We are exploring reusable GPU-driven mesh, particle, and post-processing modules: independently built features that can come together in real applications. This is a direction, not a delivery schedule or a stable module protocol. The current Reference Renderer is a teaching and showcase implementation.",
	...
	'zh-CN': {
		...
		moreExamples: "更多集成示例：",
		allExamples: "全部示例 →",
		examplesBoundary: "这些是针对特定版本的集成示例，不代表对所有引擎版本的兼容承诺。",
		inspectEyebrow: "SNAPSHOT + INSPECTOR",
		...
		runtimeBoundary: "两个运行时共享语义和可移植快照，各自提供符合语言习惯的 API；不在运行时之间直接共享 GPU 资源。",
		visionTitle: "迈向可复用的渲染模块",
		visionBody: "我们正在探索可复用的 GPU-Driven Mesh、粒子与后处理模块，让独立构建的功能更容易组合并用于实际项目。这是探索方向，不是交付时间表或稳定的模块协议。当前 Reference Renderer 用于教学与展示。",
		modelLabel: "帧内组合示意",
