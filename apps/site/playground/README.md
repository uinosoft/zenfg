...
Run these commands from the repository root. The Site owns the showcase source
...
Run `node apps/site/playground/tests/browser/layout.mjs` against the same Pages preview
...
`node apps/site/playground/tests/browser/runtimeStatus.mjs` additionally checks live FPS,
...
For browser acceptance, build with `npm run build:pages`, run
...
`apps/site/examples/typegpu-monocular-light-injection`. It lazy-loads TypeGPU inference,
...
Run `node apps/site/playground/tests/browser/runtimeOverlay.mjs` for loading, runtime output,
