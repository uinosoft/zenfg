# ZenFG 0.1.0-beta.1 release checklist
...
| 2 | `npm test` | Passed: 317/317 tests |
...
| 6 | `npm publish --dry-run --access public --tag next` for each of the three npm packages | Passed serially: Snapshot 88 files, WebGPU 93 files, Inspector 123 files |
...
| # | Check | Result |
| ---: | --- | --- |
| 16 | `npm run docs:check` validates package-root public documentation and internal documentation links | Passed: five TypeScript entrypoints, 128 exports, 38 public callable members, 68 deferred Rust top-level types, and 49 local Markdown links |
...
- `npm/inspector/v0.1.0-beta.1`
- `cargo/zenfg-snapshot/v0.1.0-beta.1`
- `cargo/zenfg/v0.1.0-beta.1`
...
4. Publish `@zenfg/snapshot@0.1.0-beta.1` under the npm `next` tag.
5. Publish `zenfg-snapshot@0.1.0-beta.1`, then wait until it is resolvable from the
   crates.io index.
...
- npm versions should be deprecated if a faulty immutable tarball escaped;
  publish the fix as `0.1.0-beta.2`.
